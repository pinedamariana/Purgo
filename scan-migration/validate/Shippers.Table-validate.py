...
df = pd.read_sql('select top 0 * from shippers', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from shippers', sqlServerEngine)
...
metatable = metadata.tables.get("shippers")
databricksCols = list()
