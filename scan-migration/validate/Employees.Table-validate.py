...
df = pd.read_sql('select top 0 * from employees', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from employees', sqlServerEngine)
...
metatable = metadata.tables.get("employees")
databricksCols = list()
