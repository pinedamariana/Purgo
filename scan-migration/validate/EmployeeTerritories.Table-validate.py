...
df = pd.read_sql('select top 0 * from employeeterritories', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from employeeterritories', sqlServerEngine)
...
metatable = metadata.tables.get("employeeterritories")
databricksCols = list()
