...
df = pd.read_sql('select top 0 * from customerdemographics', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from customerdemographics', sqlServerEngine)
...
metatable = metadata.tables.get("customerdemographics")
databricksCols = list()
