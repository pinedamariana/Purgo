...
df = pd.read_sql('select top 0 * from orders', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from orders', sqlServerEngine)
...
metatable = metadata.tables.get("orders")
databricksCols = list()
