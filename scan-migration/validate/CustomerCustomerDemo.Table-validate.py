...
df = pd.read_sql('select top 0 * from customercustomerdemo', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from customercustomerdemo', sqlServerEngine)
...
metatable = metadata.tables.get("customercustomerdemo")
databricksCols = list()
