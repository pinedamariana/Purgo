...
df = pd.read_sql('select top 0 * from products', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from products', sqlServerEngine)
...
metatable = metadata.tables.get("products")
databricksCols = list()
