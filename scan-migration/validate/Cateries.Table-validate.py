...
df = pd.read_sql('select top 0 * from cateries', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from cateries', sqlServerEngine)
...
metatable = metadata.tables.get("cateries")
databricksCols = list()
