...
df = pd.read_sql('select top 0 * from order details', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from order details', sqlServerEngine)
...
metatable = metadata.tables.get("order details")
databricksCols = list()
