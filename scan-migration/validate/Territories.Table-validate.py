...
df = pd.read_sql('select top 0 * from territories', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from territories', sqlServerEngine)
...
metatable = metadata.tables.get("territories")
databricksCols = list()
