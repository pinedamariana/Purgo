...
df = pd.read_sql('select top 0 * from suppliers', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from suppliers', sqlServerEngine)
...
metatable = metadata.tables.get("suppliers")
databricksCols = list()
