...
df = pd.read_sql('select top 0 * from customers', sqlServerEngine)
sqlServerCols = sorted(df)

df = pd.read_sql('select count(*) as COUNT from customers', sqlServerEngine)
...
metatable = metadata.tables.get("customers")
databricksCols = list()
